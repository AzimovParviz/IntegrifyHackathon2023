...
export const fetchAllprojects = createAsyncThunk(
  ...
  async () => {
    try {
      const res: AxiosResponse<Project[] | Error, any> =
...
export const fetchSingleProject = createAsyncThunk(
  ...
  async (params: { id: string }) => {
    try {
      const res: AxiosResponse<Project | Error, any> = await axiosInstance.get(
        `projects/${params.id}`
      );
      if (!(res.data instanceof Error)) return res.data;
    } catch (e) {
...
export const addProject = createAsyncThunk(
  ...
  async (project: any) => {
    try {
      const res: AxiosResponse<Project | Error, any> = await axiosInstance.post(
        "projects/",
        {},
        project
      );
      if (!(res.data instanceof Error)) {
        console.log("posted project: ", res.data);
        return project;
      }
    } catch (e) {
      return;
    ...
    build
      .addCase(fetchAllprojects.fulfilled, (state, action) => {
        if (action.payload instanceof AxiosError || !action.payload) {
          ...
          console.log(
            "There is something wrong when fetching data. Please refresh the page."
          );
          return state;
      ...
      .addCase(deleteProject.fulfilled, (state, action) => {
        if (action.payload instanceof AxiosError || !action.payload) {
          ...
          const filteredProject = state.allProjects.filter(
